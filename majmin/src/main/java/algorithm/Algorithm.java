...
public class Algorithm implements AlgorithmStep {
    ...
    public AlgorithmStepStatus getStatus() {
        AlgorithmStepStatus status = new AlgorithmStepStatus(
                "Main Algorithm: currently on step: " + currentStep.getStatus().getAlgorithmStepName());
        status.setMessage(currentStep.getStatus().getMessage());
        status.setProgress(
                ( currentStepIndex * 1.0f / steps.size() ) + ( currentStep.getStatus().getProgress() / steps.size() ));
        return status;
    }
